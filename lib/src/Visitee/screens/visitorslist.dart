...
import 'package:sc/src/Visitee/screens/visitordetails.dart';
...
}  late final List<visitorlistmodelvisitee> todos;


class visitorlistScreenState extends State<visitorlist> {
  int currentIndex = 0;
  Future<visitorlistmodelvisitee> ? visitlist;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: SideDrawer(),
      body:   FutureBuilder<List<visitorlistmodelvisitee>>(
    future: getvisitlist2(visiteeID!, context),
    builder: (context, snapshot) {
    if (snapshot.hasData) {
    List<visitorlistmodelvisitee>? data = snapshot.data;
